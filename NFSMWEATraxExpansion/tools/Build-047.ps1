...
if(-not $SkipRuntime) {
 & python -m PyInstaller --noconfirm --clean --distpath (Join-Path $root 'staging\eatrax-047-runtime') --workpath (Join-Path $root 'staging\eatrax-047-build') (Join-Path $project 'runtime\BuildCache.spec')
 if($LASTEXITCODE){throw 'Cache runtime build failed'}
}

if(-not $SkipRuntime) {
 $internal=Join-Path $root 'staging\eatrax-047-runtime\BuildCache\_internal'
 $library=Join-Path $internal 'base_library.zip'
 if(Test-Path -LiteralPath $library) {
  Add-Type -AssemblyName System.IO.Compression.FileSystem
  [System.IO.Compression.ZipFile]::ExtractToDirectory($library,$internal)
